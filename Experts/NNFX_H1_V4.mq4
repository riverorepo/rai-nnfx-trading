//+------------------------------------------------------------------+
//|                                                NNFX_H1_V4.mq4   |
//|         H1 MTF Strategy — V4 Walk-Forward Validated              |
//|         HTF: H4 McGinley(7, 0.65) trend filter                  |
//|         Entry: H1 Keltner(22) midline flip                      |
//|         Confirm: H1 T3(3, 0.90)                                 |
//|         Volume: None                                             |
//|         Exit: H1 RangeFilter(15, 3.00) flip against trade       |
//|         Magic 77704                                              |
//+------------------------------------------------------------------+
...
input int      InpMagicNumber   = 77704;       // Magic Number (H1 V4)
input double   InpRiskPercent   = 5.0;         // Risk % of Account Balance
...
input int      InpMcGinleyPer   = 7;          // McGinley Period
input double   InpMcGinleyK     = 0.65;       // McGinley Constant

//--- Entry Trigger: H1 Keltner Channel midline flip
input string   _entry_sep_      = "=== Entry: H1 Keltner Channel ==="; // ---
input int      InpKeltnerMA     = 22;         // Keltner EMA Period
input int      InpKeltnerATR    = 14;         // Keltner ATR Period
input double   InpKeltnerMult   = 1.0;        // Keltner ATR Multiplier

//--- Confirmation: H1 T3
input string   _conf_sep_       = "=== Confirm: H1 T3 ===";         // ---
input int      InpT3Period      = 3;          // T3 Period
input double   InpT3VolFactor   = 0.90;       // T3 Volume Factor (0.0-1.0)

//--- Exit: H1 RangeFilter
input string   _exit_sep_       = "=== Exit: H1 RangeFilter ===";   // ---
input int      InpRFExitPer     = 15;         // RangeFilter Period
input double   InpRFExitMult    = 3.0;        // RangeFilter Multiplier

//--- Trade management
input string   _trade_sep_      = "=== Trade Management ===";        // ---
input int      InpATRPeriod     = 14;         // ATR Period
input double   InpSLMult        = 1.65;       // SL ATR Multiplier
input double   InpTP1Mult       = 2.15;       // TP1 ATR Multiplier
...
int OnInit()
{
   ...
   Log("Initialized V4 strategy on " + Symbol());
   Log("HTF: H4 McGinley(" + IntegerToString(InpMcGinleyPer) + "," + DoubleToStr(InpMcGinleyK,2) + ") | " +
       "Entry: Keltner(" + IntegerToString(InpKeltnerMA) + ") | " +
       "Confirm: T3(" + IntegerToString(InpT3Period) + "," + DoubleToStr(InpT3VolFactor,2) + ") | " +
       "Exit: RangeFilter(" + IntegerToString(InpRFExitPer) + "," + DoubleToStr(InpRFExitMult,1) + ")");
   Log("Risk: " + DoubleToStr(InpRiskPercent, 1) + "% | SL: " + DoubleToStr(InpSLMult, 2) +
       "x ATR | TP1: " + DoubleToStr(InpTP1Mult, 2) + "x ATR");
...
// H4 McGinley Dynamic direction (signal buffer)
...
// H1 T3 direction (confirmation)
int GetT3Dir(int shift)
{
   double sig = iCustom(NULL, PERIOD_H1, IND_T3,
                          InpT3Period, InpT3VolFactor, PRICE_CLOSE, 2, shift);
   if(sig > 0.5)  return 1;
   if(sig < -0.5) return -1;
   return 0;
}

// H1 RangeFilter direction (exit)
int GetRFExitDir(int shift)
{
   double sig = iCustom(NULL, PERIOD_H1, IND_RANGEFILT,
                          InpRFExitPer, InpRFExitMult, 2, shift);
   if(sig > 0.5)  return 1;
   if(sig < -0.5) return -1;
   return 0;
}
...
int EvaluateEntry()
{
   ...
   // 3. Confirmation — T3 must agree (current or previous bar)
   int t3Curr = GetT3Dir(bar);
   int t3Prev = GetT3Dir(bar + 1);
   if(t3Curr != htfDir && t3Prev != htfDir)
   {
      Log("REJECTED: T3 disagrees (curr=" + IntegerToString(t3Curr) +
          " prev=" + IntegerToString(t3Prev) + ")");
      return 0;
   }

   // 4. No volume filter

   Log("ALL CONDITIONS MET. V4 H1 signal: " + (htfDir == 1 ? "BUY" : "SELL"));
   return htfDir;
}
...
void ExecuteEntry(int direction)
{
   ...
   int ticket1 = OrderSend(Symbol(), orderType, halfLot, (orderType == OP_BUY) ? Ask : Bid,
                             InpSlippage, sl, tp1, "NNFXV4_TP1", InpMagicNumber, 0, arrowColor);
   if(ticket1 < 0)
   {
   ...
   int ticket2 = OrderSend(Symbol(), orderType, halfLot, (orderType == OP_BUY) ? Ask : Bid,
                             InpSlippage, sl, 0, "NNFXV4_Runner", InpMagicNumber, 0, arrowColor);
   if(ticket2 < 0)
   {
...
void ManageOpenTrades()
{
   ...
   // Exit: RangeFilter flip against trade -> close runner
   if(g_order2Ticket > 0 && IsOrderOpen(g_order2Ticket))
   {
      int exitDir = GetRFExitDir(1);
      if(g_tradeDir == 1 && exitDir == -1)
      { Log("EXIT: RangeFilter flipped bearish. Closing runner."); CloseOrderByTicket(g_order2Ticket); }
      else if(g_tradeDir == -1 && exitDir == 1)
      { Log("EXIT: RangeFilter flipped bullish. Closing runner."); CloseOrderByTicket(g_order2Ticket); }
   }
}
...
void RecoverOrderState()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(StringFind(comment, "NNFXV4_TP1") >= 0) tp = OrderTicket();
      else if(StringFind(comment, "NNFXV4_Runner") >= 0) runner = OrderTicket();
...
void DrawDashboard()
{
   ...
   CreateLabel("Dash_Title", "NNFX H1 V4 | " + Symbol(), xStart, yStart + lineH * row, clrCyan, 10);
   row++;
   ...
   CreateLabel("Dash_HTF", "HTF H4 McGinley(" + IntegerToString(InpMcGinleyPer) + "," + DoubleToStr(InpMcGinleyK,2) + "): " + htfStr,
   ...
   // Confirm T3
   int t3Dir = GetT3Dir(bar);
   string t3Str = (t3Dir == 1) ? "BULL" : (t3Dir == -1) ? "BEAR" : "FLAT";
   bool t3Match = (t3Dir == htfDir && htfDir != 0);
   CreateLabel("Dash_Conf", "Confirm T3(" + IntegerToString(InpT3Period) + "," + DoubleToStr(InpT3VolFactor,2) + "): " + t3Str,
               xStart, yStart + lineH * row, t3Match ? clrLime : clrOrangeRed, 9);
   row++;

   // Exit RangeFilter
   int rfDir = GetRFExitDir(bar);
   string rfStr = (rfDir == 1) ? "BULL" : (rfDir == -1) ? "BEAR" : "FLAT";
   CreateLabel("Dash_Exit", "Exit RF(" + IntegerToString(InpRFExitPer) + "," + DoubleToStr(InpRFExitMult,1) + "): " + rfStr,
